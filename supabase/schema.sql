...
-- v27 artist landing page: optional public social audience counts
...
alter table public.profiles add constraint profiles_instagram_followers_check check (instagram_followers is null or (instagram_followers >= 0 and instagram_followers <= 1000000000000));
alter table public.profiles drop constraint if exists profiles_youtube_subscribers_check;
alter table public.profiles add constraint profiles_youtube_subscribers_check check (youtube_subscribers is null or (youtube_subscribers >= 0 and youtube_subscribers <= 1000000000000));
grant select on public.profiles to anon, authenticated;
grant insert, update on public.profiles to authenticated;
notify pgrst, 'reload schema';
