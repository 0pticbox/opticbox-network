...
let pipStream=null,pipFallbackWindow=null;
...
async function desktop(){
  try{
    ...
    stream.getTracks().forEach(q=>q.addEventListener("ended",()=>stop()));
    status(true,"Desktop audio connected");
  }catch(e){
    stop(false);
...
async function startRecording(){
  try{
    if(!canvas.captureStream||typeof MediaRecorder==="undefined")throw new Error("Canvas recording is not available in this browser. Update Firefox/Chrome/Edge and try again.");
    const preset=getRecordingPreset();
...
function toggleFirefoxFloatingWindow(){
  if(pipFallbackWindow&&!pipFallbackWindow.closed){
    ...
    status(Boolean(an),an?"Audio connected":"Floating visual closed");
    return;
  }
  const popup=window.open("","opticboxVisualizerFloat","popup=yes,width=960,height=540,resizable=yes,scrollbars=no");
  if(!popup)throw new Error("Firefox blocked the floating visual window. Allow pop-ups for this site and try again.");
  pipFallbackWindow=popup;
  popup.document.open();
  popup.document.write(`<!doctype html><html><head><title>0PS3 Floating Visual</title><style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#020309}canvas{display:block;width:100%;height:100%}</style></head><body><canvas id="floatCanvas"></canvas><script>(function(){const out=document.getElementById('floatCanvas'),ctx=out.getContext('2d');function draw(){if(!window.opener||window.opener.closed)return window.close();const src=window.opener.document.getElementById('visualizer');if(src){const r=out.getBoundingClientRect(),d=Math.min(window.devicePixelRatio||1,1.5);const w=Math.max(2,Math.floor(r.width*d)),h=Math.max(2,Math.floor(r.height*d));if(out.width!==w||out.height!==h){out.width=w;out.height=h}ctx.drawImage(src,0,0,out.width,out.height)}requestAnimationFrame(draw)}draw()})();<\/script></body></html>`);
  popup.document.close();
  pipButton.textContent="Close Floating Visual";
  popup.addEventListener("beforeunload",()=>{if(pipFallbackWindow===popup){pipFallbackWindow=null;pipButton.textContent="Picture in Picture"}});
  status(Boolean(an),"Firefox floating visual opened");
}

async function togglePictureInPicture(){
  try{
    if(pipFallbackWindow&&!pipFallbackWindow.closed){
      toggleFirefoxFloatingWindow();
      return;
    }
    ...
    const standardPip=Boolean(document.pictureInPictureEnabled&&typeof pipVideo.requestPictureInPicture==="function");
    if(!standardPip){
      toggleFirefoxFloatingWindow();
      return;
    }
    if(!canvas.captureStream)throw new Error("Canvas streaming is not supported in this browser.");
    if(!pipStream){
...
window.addEventListener("beforeunload",()=>{try{if(pipFallbackWindow&&!pipFallbackWindow.closed)pipFallbackWindow.close()}catch{}});
