...
function updateBridgeUi(){
  ...
  if(!box||!text)return;
  const connected=Boolean(b?.connected),phase=b?.phase||"idle";
  ...
  if(bridgeMode){currentInput=connected?"0PTIC'S AUDIO BRIDGE":"BRIDGE WAITING";ui.inputReadout.textContent=currentInput;status(connected?"COMPUTER OUTPUT ACTIVE":phase==="blocked"?"FIREFOX LOCAL ACCESS BLOCKED":"PRESS CONNECT BRIDGE",connected)}
}
function useBridge(){disconnectInput();bridgeMode=true;window.OpticsAudioBridge?.connect?.();updateBridgeUi()}
