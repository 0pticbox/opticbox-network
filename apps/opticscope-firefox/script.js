...
let recorder,recordChunks=[],recordStartedAt=0,recordTimer,outputCanvas,outputCtx,currentInput="NO INPUT",pngSpinActive=false,pngSpinAngle=0,pngSignalIndex=0,lastFrameTime=performance.now(),zSpinAngle=0,bridgeMode=true;
...
function updateBridgeUi(){
  const b=window.OpticsAudioBridge?.state,box=$("bridgeStatus"),text=$("bridgeStatusText");
  if(!box||!text)return;
  const connected=Boolean(b?.connected);
  ...
  if(bridgeMode){currentInput=connected?"0PTIC'S AUDIO BRIDGE":"BRIDGE WAITING";ui.inputReadout.textContent=currentInput;status(connected?"COMPUTER OUTPUT ACTIVE":"START 0PTIC'S AUDIO BRIDGE",connected)}
}
function useBridge(){disconnectInput();bridgeMode=true;window.OpticsAudioBridge?.reconnect?.();updateBridgeUi()}
window.addEventListener("opticbridgechange",updateBridgeUi);

async function connectMic(){bridgeMode=false;disconnectInput();await ensureAudio();const deviceId=$("firefoxAudioDevice")?.value||"";const audio={channelCount:2,echoCancellation:false,noiseSuppression:false,autoGainControl:false};if(deviceId)audio.deviceId={exact:deviceId};sourceStream=await navigator.mediaDevices.getUserMedia({audio,video:false});sourceNode=audioContext.createMediaStreamSource(sourceStream);setupAnalysis(sourceNode,sourceNode.channelCount||1);currentInput=(sourceStream.getAudioTracks()[0]?.label||"AUDIO INPUT").toUpperCase();ui.inputReadout.textContent=currentInput;status("INPUT ACTIVE");refreshFirefoxAudioDevices(false)}
...
function readAudio(){
  if(bridgeMode){
    ...
    if(bridge?.state.connected){bridge.fillFloatWave(leftData,rightData,monoData);return}
    leftData.fill(0);rightData.fill(0);monoData.fill(0);return
  }
  if(!monoAnalyser){leftData.fill(0);rightData.fill(0);monoData.fill(0);return}
  leftAnalyser.getFloatTimeDomainData(leftData);rightAnalyser.getFloatTimeDomainData(rightData);monoAnalyser.getFloatTimeDomainData(monoData)
}
