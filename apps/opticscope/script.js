...
async function connectMic(){disconnectInput();await ensureAudio();sourceStream=await navigator.mediaDevices.getUserMedia({audio:{channelCount:2,echoCancellation:false,noiseSuppression:false,autoGainControl:false}});sourceNode=audioContext.createMediaStreamSource(sourceStream);setupAnalysis(sourceNode,sourceNode.channelCount||1);currentInput="MICROPHONE";ui.inputReadout.textContent=currentInput;status("INPUT ACTIVE")}
async function connectDesktop(){disconnectInput();await ensureAudio();sourceStream=await navigator.mediaDevices.getDisplayMedia({video:{width:{ideal:320},height:{ideal:180},frameRate:{ideal:5,max:10}},audio:{echoCancellation:false,noiseSuppression:false,autoGainControl:false}});const a=sourceStream.getAudioTracks();if(!a.length){sourceStream.getTracks().forEach(t=>t.stop());throw Error("No desktop audio track. Enable system audio in the sharing dialog.")}sourceNode=audioContext.createMediaStreamSource(new MediaStream(a));setupAnalysis(sourceNode,2);currentInput="DESKTOP AUDIO";ui.inputReadout.textContent=currentInput;status("INPUT ACTIVE")}
...
async function startRecord(){const z=recordSize();outputCanvas=document.createElement("canvas");outputCanvas.width=z.width;outputCanvas.height=z.height;outputCtx=outputCanvas.getContext("2d");const stream=outputCanvas.captureStream(30),combined=new MediaStream(stream.getVideoTracks());recordDest?.stream.getAudioTracks().forEach(t=>combined.addTrack(t));recorder=new MediaRecorder(combined,{mimeType:mime(),videoBitsPerSecond:8e6});recordChunks=[];recorder.ondataavailable=e=>e.data.size&&recordChunks.push(e.data);recorder.onstop=saveRecord;recorder.start(250);recordStartedAt=Date.now();$("recordBtn").disabled=true;$("stopRecordBtn").disabled=false;ui.recordReadout.textContent="REC ACTIVE";ui.recordGuide.hidden=false;recordTimer=setInterval(recordClock,500);recordClock()}
