...
async function checkSchema() {
  ...
  schemaReady = !error;
  if (fileInput) fileInput.disabled = !schemaReady;
...
async function sendTemporaryFile(event) {
  ...
  if (!thread) return say('Choose a friend conversation before sending a file.', true);
...
async function renderExistingFileCards() {
  if (!schemaReady || !messageLog) return;
  const thread = await getCurrentThread().catch(() => null);
  if (!thread) return;

  const { data, error } = await supabase
    .from('direct_messages')
    .select('id,media_path,media_name,media_size,media_expires_at')
  ...
  for (const row of data || []) {
    ...
    if (!bubble || bubble.querySelector('.temp-file-card')) continue;
    ...
    if (!remaining) {
      ...
      const { data: signed, error: signedError } = await supabase.storage.from('message-media').createSignedUrl(row.media_path, ttl);
      if (!signedError && (signed?.signedUrl || signed?.signedURL)) {
        const link = document.createElement('a');
        link.className = 'retro-button temp-file-download';
        link.href = signed.signedUrl || signed.signedURL;
        link.target = '_blank';
        link.rel = 'noopener';
...
async function init() {
  ...
  if (!user) return;
  await checkSchema();
  window.setTimeout(() => void renderExistingFileCards(), 500);
}
